// Generated by Haxe 4.0.0-rc.3+a900e55
...
var Main = function() {
	...
	this.mousedown = this.makeSignal(window.document,"mousedown");
	this.mouseup = this.makeSignal(window.document,"mouseup");
};
Main.__name__ = true;
Main.main = function() {
...
Main.prototype = {
	init: function() {
		var _gthis = this;
		console.log("src/Main.hx:26:","init");
		this.sound = new Pizzicato.Sound({ source : "wave", options : { type : "sawtooth"}});
		var moved;
		this.mousedown.handle(function(e) {
			_gthis.sound.play();
			_gthis.mousemove = _gthis.makeSignal(window.document,"mousemove");
			moved = _gthis.mousemove.handle($bind(_gthis,_gthis.moove));
			console.log("src/Main.hx:42:","down");
			return e;
		});
		this.mouseup.handle(function(e1) {
			console.log("src/Main.hx:45:","up");
			_gthis.sound.stop();
			if(moved != null) {
				moved.cancel();
			}
	...
	,moove: function(e) {
		var scale = postite_math_Matools.scale([0,window.innerWidth],[0,1],e.clientX);
		console.log("src/Main.hx:57:",scale);
		this.sound.volume = scale;
		this.sound.frequency = postite_math_Matools.scale([0,window.innerHeight],[0,2000],e.clientY) | 0;
	}
	,makeSignal: function(dispatcher,type) {
		var f = $bind(dispatcher,dispatcher.addEventListener);
		var type1 = type;
		var f1 = $bind(dispatcher,dispatcher.removeEventListener);
		var type2 = type;
		return tink_core__$Signal_Signal_$Impl_$.ofClassical(function(listener) {
			f(type1,listener);
		},function(listener1) {
			f1(type2,listener1);
		});
